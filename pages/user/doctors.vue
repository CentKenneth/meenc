...
export default {
    ...
    data() {
        return {
            ...
            icon: {
                ...
                'degreelevel': 'mdi-stairs',
                'degreefield': 'mdi-doctor',
                'bday': 'mdi-cake-variant',
                'gender': 'mdi-gender-male-female',
                'clinicname': 'mdi-hospital-building',
                'email': 'mdi-email',
            ...
            items: {
                'degreelevel': ['Doctorate', 'Professional', 'Masters'],
                'degreefield': ['Anesthesiologists', 'Dermatologists', 'Internists', 'Obstetricians and gynecologists','Ophthalmologists', 'Pediatricians','Physicians', 'Psychiatrists', 'Surgeons'],
    ...
    methods: {
        async initialize () {
            ...
            // api request
            const doctors = await this.$axios.get(`api/authorized/get-doctors`)

            // filter doctor email
            if (doctors?.data) {
                this.doctors = []
                doctors.data.forEach(el => {
                    // push data to array
                    this.doctors.push({
                        "id": el.id,
                        "name": el.name,
            ...
            try {
                ...
                if(valid) {
                    ...
                    if(this.action == 'Add') {
                        ...
                        payload.append('degreelevel', this.form.degreelevel)
                        payload.append('degreefield', this.form.degreefield)
                        payload.append('bday', this.form.bday)
                        payload.append('gender', this.form.gender)
                        payload.append('clinicname', this.form.clinicname)
